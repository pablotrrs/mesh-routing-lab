class PacketRegistry:
    ...
    def mark_episode_complete(self, episode_number, episode_success):
        print(f"[PacketRegistry] Marking episode {episode_number} as {'successful' if episode_success else 'failed'}.")
        self.packet_log[episode_number]["episode_success"] = episode_success

    def log_packet_hop(self, episode_number, from_node_id, to_node_id, function, node_status, latency, packet_type):
        self.packet_log[episode_number]["route"].append({
            "from": from_node_id,
            "to": to_node_id,
            "function": function,
